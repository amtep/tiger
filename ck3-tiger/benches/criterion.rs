...
use tiger_lib::{Everything, Fileset, ModFile};
...
fn bench_mod(vanilla_dir: &Path, modfile_path: &PathBuf) {
    let mut fileset = Fileset::new(Some(vanilla_dir), modfile_path.clone());
    let modfile = ModFile::read(&mut fileset, modfile_path).unwrap();
