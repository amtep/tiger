...
pub struct Localization {
    ...
    check_langs: BitArr!(for Language::COUNT, in u16, Lsb0),
    ...
    mod_langs: BitArr!(for Language::COUNT, in u16, Lsb0),
...
impl Localization {
    ...
    fn iter_lang(&self) -> impl Iterator<Item = Language> + '_ {
        self.iter_lang_idx().map(Language::from_idx)
    }
    ...
    pub fn verify_exists_implied(&self, key: &str, token: &Token, max_sev: Severity) {
        ...
        let langs_missing = self.mark_used_missing(key);
    ...
    pub fn verify_name_exists(&self, name: &Token, max_sev: Severity) {
        ...
        let langs_missing = self.mark_used_missing(name.as_str());
        if !langs_missing.is_empty() {
    ...
    pub fn verify_exists_implied_lang(&self, key: &str, token: &Token, lang: Option<Language>) {
        ...
        if let Some(lang) = lang {
            if !self.mark_used_lang(key, lang) {
    ...
    pub fn mark_used(&self, key: &str) -> bool {
        let mut exists = false;
        for lang in self.iter_lang() {
            exists |= self.mark_used_lang(key, lang);
        }
        exists
    }
    ...
    fn mark_used_missing(&self, key: &str) -> Vec<&'static str> {
    ...
    fn mark_used_lang(&self, key: &str, lang: Language) -> bool {
