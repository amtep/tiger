...
impl PdxFile {
    ...
    pub fn read(entry: &Arc<FileEntry>, parser: &ParserMemory) -> Option<Block> {
    ...
    #[cfg(feature = "hoi4")]
    pub fn read_no_bom(entry: &Arc<FileEntry>, parser: &ParserMemory) -> Option<Block> {
        if entry.contents()?.has_bom() {
    ...
    pub fn read_optional_bom(entry: &Arc<FileEntry>, parser: &ParserMemory) -> Option<Block> {
        parse_pdx_file(entry, parser)
    }

    /// Parse a file that may be in UTF-8 with BOM encoding, or Windows-1252 encoding.
    #[cfg(feature = "ck3")]
    pub fn read_detect_encoding(entry: &Arc<FileEntry>, parser: &ParserMemory) -> Option<Block> {
    ...
    #[cfg(feature = "ck3")]
    pub fn reader_export(entry: &Arc<FileEntry>, memory: &mut PdxfileMemory) {
