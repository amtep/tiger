...
use crate::filecontent::FileContent;
use crate::game::Game;
...
use crate::report::{
    add_loaded_dlc_root, add_loaded_mod_root, err, fatal, report, store_source_file, ErrorKey,
    Severity,
};
...
impl FileEntry {
    ...
    fn new(path: PathBuf, kind: FileKind, fullpath: PathBuf) -> Arc<Self> {
...
// TODO: This should really be constructed with a builder pattern or something
#[derive(Debug)]
pub struct Fileset {
    ...
    /// The CK3 and mod files in arbitrary order.
    files: TigerHashMap<PathBuf, Arc<FileEntry>>,

    /// The CK3 and mod files in the order the game would load them.
    ordered_files: Vec<Arc<FileEntry>>,
    ...
    filenames: TigerHashMap<PathBuf, Arc<FileEntry>>,
...
impl Fileset {
    pub fn new(vanilla_dir: Option<&Path>, mod_root: PathBuf) -> Self {
        ...
        Fileset {
            ...
            #[cfg(feature = "jomini")]
            jomini_root,
            the_mod: LoadedMod::new_main_mod(mod_root, vec![]),
            loaded_mods: Vec::new(),
            loaded_dlcs: Vec::new(),
            config: None,
            files: TigerHashMap::default(),
            ordered_files: Vec::new(),
            filename_tokens: Vec::new(),
            filenames: TigerHashMap::default(),
            directories: RwLock::new(TigerHashSet::default()),
            used: RwLock::new(TigerHashSet::default()),
        }
    }

    pub fn set_replace_paths(&mut self, replace: Vec<PathBuf>) {
        self.the_mod.replace_paths = replace;
    }
        ...
        for block in config.get_field_blocks("load_mod") {
            ...
            if Game::is_ck3() || Game::is_imperator() || Game::is_hoi4() {
                ...
                if let Some(path) = get_modfile(&label, config_path, block, paradox_dir) {
                    let modfile = ModFile::read(self, &path)?;
                    eprintln!(
                ...
                if let Some(pathdir) = get_mod(&label, config_path, block, workshop_dir) {
                    match ModMetadata::read(self, &pathdir) {
                        Ok(metadata) => {
                            eprintln!(
                                "Loading secondary mod {label} from: {}{}",
                                pathdir.display(),
    ...
    pub fn is_finalized(&self) -> bool {
        !self.ordered_files.is_empty()
    }

    fn path_to_files_key(path: &Path) -> PathBuf {
    ...
    pub fn get_or_create_entry(
        ...
        let key = Self::path_to_files_key(&path);
        if self.is_finalized() {
            self.files.get(&key)
        } else {
            Some(self.files.entry(key).or_insert_with(|| FileEntry::new(path, kind, fullpath)))
        }
    }
    ...
    fn scan(&mut self, path: &Path, kind: FileKind) -> Result<(), walkdir::Error> {
        for entry in WalkDir::new(path) {
            ...
            self.files.insert(
                Self::path_to_files_key(inner_path),
    ...
    pub fn finalize(&mut self) {
        ...
        let sorted = self.files.values().sorted();
            ...
            if let Some(prev) = self.ordered_files.last_mut() {
                if entry.path == prev.path {
                    *prev = Arc::clone(entry);
                    ...
                    self.ordered_files.push(Arc::clone(entry));
                }
            } else {
                self.ordered_files.push(Arc::clone(entry));
            }
        ...
        for entry in &self.ordered_files {
            ...
            self.filenames.insert(entry.path.clone(), Arc::clone(entry));
        }
    }
